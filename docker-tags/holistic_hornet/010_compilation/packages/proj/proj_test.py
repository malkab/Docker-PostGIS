...
# Works with the PROJ Degree, minutes, second format
...
class PROJDms:
    ...
    def compare(self, coord, tolerance = [ 0, 0, 0 ], heightError = None):
        ...
        # For comparison DMS <> DMS

        if self.cType == "D" and coord.cType == "D":

            if abs(self.x[0] - coord.x[0]) > 0:

                raise Exception("PROJDms comparison error: longitude degrees mismatch for %s <> %s: %s, %s" % (str(self), str(coord), self.x[0], coord.x[0]))


            if abs(self.y[0] - coord.y[0]) > 0:

                raise Exception("PROJDms comparison error: latitude degrees mismatch for %s <> %s: %s, %s" % (str(self), str(coord), self.y[0], coord.y[0]))


            if abs(self.x[1] - coord.x[1]) > 0:

                raise Exception("PROJDms comparison error: longitude minutes mismatch for %s <> %s: %s, %s" % (str(self), str(coord), self.x[1], coord.x[1]))


            if abs(self.y[1] - coord.y[1]) > 0:

                raise Exception("PROJDms comparison error: latitude minutes mismatch for %s <> %s: %s, %s" % (str(self), str(coord), self.y[1], coord.y[1]))


            if abs(self.x[2] - coord.x[2]) > tolerance[0]:

                raise Exception("PROJDms comparison error: longitude seconds mismatch for %s <> %s: %s, %s" % (str(self), str(coord), self.x[2], coord.x[2]))


            if abs(self.y[2] - coord.y[2]) > tolerance[0]:

                raise Exception("PROJDms comparison error: latitude seconds mismatch for %s <> %s: %s, %s" % (str(self), str(coord), self.x[2], coord.x[2]))
            ...
            if self.z and coord.z and heightError:
                ...
                if abs(self.z - coord.z) > tolerance[2]:
                    ...
                    raise Exception("PROJDms comparison error: height mismatch for %s <> %s: %s, %s" % (str(self), str(coord), self.z, coord.z))
            ...
            if abs(self.x - coord.x) > tolerance[1]:
                ...
                raise Exception("PROJDms comparison error: x mismatch for %s <> %s: %s, %s" % (str(self), str(coord), self.x, coord.x))


            if abs(self.y - coord.y) > tolerance[1]:

                raise Exception("PROJDms comparison error: y mismatch for %s <> %s: %s, %s" % (str(self), str(coord), self.y, coord.y))
            ...
            raise Exception("PROJDms comparison error: cannot compare coords in DMS and UTM")
...
for f in srs:
    ...
    for t in srs:
        ...
        for p in range(len(f["points"])):
            ...
            toP = PROJDms(t["points"][p]["point"])

            trans = PROJDms(cs2csTransform(f["epsg"], t["epsg"], f["points"][p]["point"]))
