...
*/
create or replace function public.gs__test_grid_proj()
returns boolean as
$$
declare
    _sql text;
...
begin
    ...
    if not round(st_x(_tp)::numeric,3) between _t23030x-0.1 and _t23030x+0.1 then
    ...
    if not round(st_y(_tp)::numeric,3) between _t23030y-0.1 and _t23030y+0.1 then
...
\echo Testing proj NTV2 support for Spain...

select public.gs__test_grid_proj();
